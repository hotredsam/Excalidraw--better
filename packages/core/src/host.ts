 ...
 */
export function defaultHostServices(overrides: HostServicesOptions = {}): HostServices {
  ...
  return {
    ...
    trashItem: overrides.trashItem ?? (async (targetPath: string) => { await fs.remove(targetPath); }),
    pickDirectory: overrides.pickDirectory ?? (async () => null),
    pickFile: overrides.pickFile ?? (async () => null),
    openExternal: overrides.openExternal ?? (async () => { /* no-op */ }),
  };
}
