...
import { AppPingSchema, ProfileSchema, ProfileListSchema, SettingsSchema, WorkspaceSchema, WorkspaceListSchema, PluginListSchema, validateRawPayload, AiPayloadSchema, TemplateListSchema, RecentFileListSchema, LibraryListSchema, LibrarySchema, BulkRenameOptionsSchema, WorkspaceStatsSchema, MarkdownOptionsSchema, } from '@excalibur/shared';
 ...
 */
export function createApiHandlers(engine, host = engine.host) {
    return {
        app: {
            ping: async () => AppPingSchema.parse({ ok: true, version: host.appVersion, platform: host.platform }),
        },
        profiles: {
            ...
            create: async (name) => ProfileSchema.parse(await engine.profileStore.create(name)),
            rename: async (id, name) => ProfileSchema.parse(await engine.profileStore.rename(id, name)),
            delete: async (id) => {
        ...
        settings: {
            ...
            update: async (partial) => SettingsSchema.parse(await engine.settingsStore.update(partial)),
        },
        workspaces: {
            ...
            listFiles: async (workspaceId, subDir = '') => {
                ...
                if (!isPathWithin(workspace.path, targetDir) &&
        ...
        ai: {
            validate: async (raw) => validateRawPayload(raw || ''),
            apply: async (payload) => {
                ...
                return await applyAiPayload(profileDir, parsed, (partial) => engine.settingsStore.update(partial).then(() => { }));
            },
        },
        templates: {
        ...
        recents: {
            ...
            add: async (entry) => RecentFileListSchema.parse({ recents: await engine.recentsStore.add(entry) }),
            remove: async (filePath) => RecentFileListSchema.parse({ recents: await engine.recentsStore.remove(filePath) }),
            clear: async () => {
        ...
        libraries: {
            ...
            get: async (id) => LibrarySchema.parse(await engine.libraryStore.get(id)),
            import: async () => {
        ...
        markdown: {
            export: async (workspaceId, baseName, options, imageData, scene, bodyText) => {
                const workspace = await engine.getWorkspaceOrThrow(workspaceId);
                const bundle = await writeMarkdownBundle(workspace.path, baseName, MarkdownOptionsSchema.parse(options), imageData, scene, bodyText || '');
                engine.getIndex(workspace.path).invalidate();
                return bundle;
            },
        },
