...
exports.applyRenameTemplate = applyRenameTemplate;
exports.extractSlidesFromScene = extractSlidesFromScene;
 ...
 */
function applyRenameTemplate(template, ctx) {
 ...
 */
function extractSlidesFromScene(scene) {
...
// ── Markdown export ───────────────────────────────────────────────────────
function buildMarkdown(opts, imageRelPath, bodyText = '') {
    const lines = [];
    ...
    if (opts.includeFrontmatter) {
        ...
        if (opts.tags.length)
            lines.push(`tags: [${opts.tags.map((t) => `"${t}"`).join(', ')}]`);
...
function parseGitStatus(porcelain) {
    ...
    const status = { isRepo: true, ahead: 0, behind: 0, files: [], clean: true };
    for (const line of lines) {
        if (line.startsWith('## ')) {
            ...
            if (ahead)
                status.ahead = parseInt(ahead[1], 10);
            if (behind)
                status.behind = parseInt(behind[1], 10);
            continue;
        }
        const index = line[0];
        const working = line[1];
        const path = line.slice(3);
        const f = { path, index: index.trim(), working: working.trim() };
        status.files.push(f);
    }
