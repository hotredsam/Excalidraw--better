...
export function presetToAppState(preset: StylePreset): Record<string, any> {
  ...
  if (preset.fontFamily !== undefined) out.currentItemFontFamily = preset.fontFamily;
  return out;
}

/** Extract a style preset shape from an Excalidraw appState. */
export function appStateToPreset(appState: Record<string, any>): Omit<StylePreset, 'id' | 'name'> {
