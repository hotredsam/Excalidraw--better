 ...
 */
export function parseTextPayload(text: string): Record<string, any> {
  ...
  for (const raw of lines) {
    ...
    if (bullet && currentListKey) {
      (result[currentListKey] as string[]).push(bullet[1].trim());
      continue;
    }
 ...
 */
export function normalizeRawPayload(raw: string): Record<string, any> | null {
...
export function validateRawPayload(raw: string): AiValidationResult {
  ...
  if (!AI_PAYLOAD_TYPES.includes((normalized.type as AiPayloadType))) {
    return {
      ok: false,
      summary: [],
      errors: [
  ...
  if (!result.success) {
    return {
      ok: false,
      type: normalized.type as AiPayloadType,
      summary: [],
...
export function summarizePayload(payload: AiPayload): string[] {
  switch (payload.type) {
