...
exports.parseSvgToElements = parseSvgToElements;
...
function parseSvgToElements(svg) {
    const elements = [];
    let skipped = 0;
    ...
    for (const tag of tags) {
        ...
        if (kind === 'rect') {
            ...
            const x2 = num(tag, 'x2');
            const y2 = num(tag, 'y2');
            const el = baseElement('line', Math.min(x1, x2), Math.min(y1, y2), Math.abs(x2 - x1), Math.abs(y2 - y1), s);
            el.points = [
                [0, 0],
                [x2 - x1, y2 - y1],
            ];
            ...
            const el = baseElement('text', num(tag, 'x'), num(tag, 'y'), Math.max(20, text.length * 8), 20, s);
            el.text = text;
