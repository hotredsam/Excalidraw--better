...
import { Slide, SlideDeck, extractSlidesFromScene } from '@excalibur/shared';
...
function notesFile(workspacePath: string, fileRel: string): string {
  const safe = fileRel.replace(/[\\/]/g, '__');
  return path.join(workspacePath, '.excalibur', 'slides', `${safe}.json`);
}

export function buildDeck(scene: any): SlideDeck {
  return { slides: extractSlidesFromScene(scene) };
}

export async function getDeck(workspacePath: string, fileRel: string, scene: any): Promise<SlideDeck> {
  const deck = buildDeck(scene);
  const nf = notesFile(workspacePath, fileRel);
  if (await fs.pathExists(nf)) {
    try {
      const notes: Record<string, string> = await fs.readJson(nf);
...
export async function setSlideNotes(
  ...
  if (notes.trim()) all[slideId] = notes;
  else delete all[slideId];
  await writeJsonAtomic(nf, all);
}

/** Compute an export viewport (with padding) for a single slide. */
export function slideViewport(slide: Slide, padding = 20) {
  return {
    x: slide.x - padding,
    y: slide.y - padding,
