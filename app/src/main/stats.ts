 ...
 */
export async function computeStats(workspacePath: string, tags: FileTags = {}): Promise<WorkspaceStats> {
  const byExtension: Record<string, number> = {};
  let totalFiles = 0;
  let totalBytes = 0;
  let totalElements = 0;
  const all: { name: string; path: string; size: number; mtime: number }[] = [];
    ...
    for (const item of items) {
      ...
      if (ext === '.excalidraw' || ext === '.json') {
        try {
          ...
          if (Array.isArray(scene?.elements)) totalElements += scene.elements.length;
        } catch {
  ...
  for (const list of Object.values(tags)) {
    for (const t of list) tagHistogram[t] = (tagHistogram[t] || 0) + 1;
  }
