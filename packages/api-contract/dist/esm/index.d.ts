import type { AppPing, Profile, ProfileList, Settings, Workspace, WorkspaceList, FileInfo, ExcalidrawFile, InstalledPlugin, PluginList, PluginContributes, AiValidationResult, AiApplyResult, AiPayload, SearchResultList, FileTags, TemplateList, TemplateSummary, StoredTemplate, RecentFileList, RecentFile, LibraryList, Library, LibrarySummary, BulkResult, BulkRenameOptions, SlideDeck, Review, WorkspaceStats, GitStatus, Command, BackupList, MarkdownOptions, SnippetList, Snippet, SnippetSummary, ShortcutBinding, WorkspaceConfig, StylePreset } from '@excalibur/shared';
export type Ok = {
    success: boolean;
};
...
/**
 * The complete, transport-agnostic application API surface.
 *
 * This is the single source of truth shared by every layer:
 * - the Electron preload (`window.api`) implements it over IPC,
 * - `@excalibur/core`'s `createApiHandlers` implements it in-process,
 * - the renderer consumes it through its `ApiProvider`/`useApi()`,
 * - the renderer test double (`fakeApi`) conforms to it.
 *
 * Any drift between these is a compile error. `onMenuCommand` is optional
 * because it is an Electron-only menu/keyboard event bridge that headless or
 * web hosts need not provide.
 */
export interface ExcaliburApi {
    app: {
    ...
    workspaces: {
        ...
        exportFile: (workspaceId: string, filePath: string, format: 'png' | 'svg' | 'json', data: string, scene: any) => Promise<{
            success: boolean;
            path: string;
        }>;
    };
    ...
    git: {
        ...
        commit: (workspaceId: string, message: string, files?: string[]) => Promise<{
            output: string;
        }>;
        log: (workspaceId: string, limit?: number) => Promise<{
            entries: {
                hash: string;
                subject: string;
                date: string;
    ...
    backups: {
        ...
        restore: (id: string, destPath?: string) => Promise<{
            path: string;
        }>;
    };
    markdown: {
        export: (workspaceId: string, baseName: string, options: MarkdownOptions, imageData: string, scene: any, bodyText?: string) => Promise<{
            imagePath: string;
            markdownPath: string;
        }>;
    };
    ...
    shortcuts: {
        ...
        set: (commandId: string, accelerator: string, force?: boolean) => Promise<{
            bindings: ShortcutBinding[];
        }>;
        reset: (commandId?: string) => Promise<{
            bindings: ShortcutBinding[];
        }>;
    };
    workspaceConfig: {
    ...
    /** Electron-only: subscribe to menu/keyboard commands. Optional for non-Electron hosts. */
