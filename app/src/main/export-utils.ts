...
export function encodeChunks(chunks: Chunk[]): Buffer {
  ...
  for (const chunk of chunks) {
    ...
    crcBuf.writeUInt32BE(crc.crc32(Buffer.concat([name, data])) >>> 0, 0);
 ...
 */
export function embedSceneInPng(pngBuffer: Buffer, scene: unknown): Buffer {
  const chunks = extractChunks(pngBuffer) as Chunk[];
  // Drop any pre-existing excalidraw text chunk so we don't duplicate.
  const filtered = chunks.filter((c) => {
    if (c.name !== 'tEXt') return true;
    try {
      return text.decode(c.data).keyword.toLowerCase() !== 'excalidraw';
    } catch {
      return true;
    }
  });

  const sceneChunk = text.encode('excalidraw', JSON.stringify(scene)) as Chunk;
 ...
 */
export function embedSceneInSvg(svg: string, scene: unknown): string {
