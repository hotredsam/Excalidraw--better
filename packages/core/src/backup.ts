...
export class BackupManager {
  ...
  private entries: BackupEntry[] = [];

  constructor(profileDir: string, private keepPerFile = 10) {
  ...
  async backup(originalPath: string): Promise<BackupEntry | null> {
    if (!(await fs.pathExists(originalPath))) return null;
    const id = `${Date.now()}-${nanoid(6)}`;
    const base = path.basename(originalPath);
    const backupPath = path.join(this.dir, `${id}__${base}`);
    await fs.copy(originalPath, backupPath);
    const stat = await fs.stat(backupPath);
    const entry: BackupEntry = { id, originalPath, backupPath, createdAt: Date.now(), size: stat.size };
  ...
  private async prune(originalPath: string) {
  ...
  async restore(id: string, destPath?: string): Promise<string> {
